...
$actual_type = 'artist';
    ...
    elseif(isset($_POST['tourSubmit']))
    {
        $tourBangalore = isset($_POST['tourBangalore']);
        $tourDelhi = isset($_POST['tourDelhi']);
        $tourGoa = isset($_POST['tourGoa']);
        $tourBudget = $_POST['tourBudget'];
        $tourNumEvents = $_POST['tourNumEvents'];
        $tourStartDate = $_POST['tourStartDate'];
        $tourEndDate = $_POST['tourEndDate'];
        $startTime = strtotime($tourStartDate);
        $startTime = date('Y-m-d',$startTime);
        $endTime = strtotime($tourEndDate);
        $endTime = date('Y-m-d',$endTime);
        
        $newformat = date('Y-m-d',$time);
        
        $sqlS = "SELECT * FROM `$database`.`members` WHERE fb_id='$username'";
		$result_set1 = mysql_query($sqlS);
		if (mysql_num_rows($result_set1) == 1) 
		{
        ...
        $sqlI = "INSERT INTO `$database`.`tour` (`artistID`, `artistName`, `tourBangalore`, `tourGoa`, `tourDelhi`, `budget`, `numEvents`, `startDate`, `endDate`) VALUES('$aid', '$aname', '$tourBangalore', '$tourGoa', '$tourDelhi', '$tourBudget', '$tourNumEvents', '$startTime', '$endTime')";
        $result_set2 = mysql_query($sqlI);
        if ($result_set2)
        {
	/*	echo ("<SCRIPT LANGUAGE='JavaScript'> 
       ...
       //    $_SESSION['tourSubmitted'] = 1;	
		...
		// print("<script language='javascript' type='text/javascript'>alert('Your request has been submitted!');</script>"); 
