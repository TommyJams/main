	...
	/**************************************************************************/	
	function initAdvertisementCarousel() {
	$('.right-advertisement-carousel').carouFredSel(
	{
		circular		:	true,
		direction		:	'left',
                auto: 
	...
	onload=function()
	{
		...
		var rightEdgeForRightCommon = 0.98 * totalWidth;
		...
		if (lefty) {
		    lefty.style.height = leftyContainerHeight + 'px';
		    lefty.style.width  = leftyContainerWidth + 'px';
		}

		//document.getElementById('left').style.width = leftyContainerWidth +'px';

		// Align headerBox, roght side advert and copyrightcontainer.
		// We want them to be aligned on their right edge.
		// So, we give them a left absolute position of the fixed right edge - their individual widths.
		// To hide the sudden movement of elements when this left position is changed,
		// we keep these elements as hidden by default and show them after we have positioned them.
		var headerBox = document.getElementById('headerBox');
		var headerBoxWidth = $("#headerBox").outerWidth();
		headerBox.style.left   = (rightEdgeForRightCommon -  headerBoxWidth) + 'px';
		headerBox.style.display = 'block';

		var rightAdvertisementContainer = document.getElementsByClassName('right-advertisement-container')[0];
		var rightAdvertisementContainerWidth = $(".right-advertisement-container").outerWidth();
                rightAdvertisementContainer.style.left = (rightEdgeForRightCommon - rightAdvertisementContainerWidth) + 'px';
                rightAdvertisementContainer.style.top = 0.25 * totalHeight + 'px';
                rightAdvertisementContainer.style.display = 'block';
                initAdvertisementCarousel();

		var copyrightContainer = document.getElementById('copyrightContainer');
		var copyrightContainerWidth = $("#copyrightContainer").outerWidth();
		copyrightContainer.style.left = (rightEdgeForRightCommon - copyrightContainerWidth) + 'px';
		copyrightContainer.style.display = 'block';

	
		//  Google Analytics Code
		var _gaq = _gaq || [];
		_gaq.push(['_setAccount', 'UA-41581813-1']);
    ...
    function initHeader()
    {
		...
		window.fbAsyncInit = function() 
		{
			// init the FB JS SDK
			FB.init({
			  appId      : '394382993976805', // App ID from the App Dashboard
			  channelUrl : '//WWW.TOMMYJAMS.COM/PROTOTYPE/channel.html', // Channel File for x-domain communication
	...
	{
		window.location = "http://www.tommyjams.com/prototype/fbconnect.php?registered=yes";
	}
